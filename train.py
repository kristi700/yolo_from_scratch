...
def train(dataloader, model, device): # , model, criterion, optimizer, scaler
    for i, minibatch in enumerate(dataloader):
        images, labels = minibatch[0], minibatch[1]
        predicitions = model(images).to(device) # todevice kéne majdmég
        print(predicitions)
...
def main():
    ...
    for epoch in range(0, 10):
        current_loss = train(dataloader=train_loader, model=model, device=device)
